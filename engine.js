...
/* luminous edge bloom — premium glass glows softly at its silhouette; the
   glow is tinted by the accent so dark/vivid glass gets a coloured halo */
function glassGlow(g,S,path,tint,k,amount){
  const a=amount===undefined?0.5:amount;
  g.save();
  g.filter=`blur(${7*k}px)`;g.globalAlpha=a;
  g.strokeStyle=shade(tint,+0.34);g.lineWidth=6*k;g.stroke(path);
  g.filter=`blur(${3*k}px)`;g.globalAlpha=a*0.8;
  g.strokeStyle=shade(tint,+0.5);g.lineWidth=3*k;g.stroke(path);
  g.restore();
}
...
function glassThickness(g,S,path,m,k,lx,ly,thick,clarity){
  ...
  const wa=1-cl*0.85;                        /* clear glass → very faint wall */
  lx=lx===undefined?-0.55:lx; ly=ly===undefined?-0.62:ly;
...
function glassGloss(g,S,path,m,k,lx,ly,frost,clarity){
  lx=lx===undefined?-0.55:lx; ly=ly===undefined?-0.62:ly;
  const fr=frost===undefined?0:frost;
  const cl=clarity===undefined?0.5:clarity;
  /* the FACE reflection fades as the glass clears — clear glass keeps only a
     whisper of reflection so the body stays fully see-through (edges glow
     separately in glassThickness). r = reflection strength. */
  const r=(1-0.5*fr)*(0.35+0.65*(1-cl));
  g.save();g.clip(path);
  const gx=m.x+m.w*(0.5+lx*0.24), gy=m.y+m.h*(0.13+Math.max(0,-ly-0.3)*0.1);
  /* thin top-edge reflection */
  const gl=g.createLinearGradient(0,m.y-2*k,0,m.y+m.h*0.2);
  gl.addColorStop(0,`rgba(255,255,255,${0.4*r})`);
  gl.addColorStop(0.5,`rgba(255,255,255,${0.05*r})`);
  gl.addColorStop(1,"rgba(255,255,255,0)");
  g.fillStyle=gl;g.fillRect(m.x-4*k,m.y-4*k,m.w+8*k,m.h*0.24);
  /* small refined wet glint near the top-light corner */
  g.filter=`blur(${(0.5+5*fr)*k}px)`;
  g.fillStyle=`rgba(255,255,255,${0.9*r})`;
  g.beginPath();g.ellipse(gx,gy,m.w*0.07,m.h*0.024,-0.6,0,Math.PI*2);g.fill();
  g.fillStyle=`rgba(255,255,255,${0.6*r})`;
  g.beginPath();g.ellipse(gx+m.w*0.07,gy+m.h*0.008,m.w*0.022,m.h*0.013,-0.6,0,Math.PI*2);g.fill();
  g.filter="none";
  g.restore();
}
...
function renderGlassBody(g,S,path,cA,cB,cC,mode,shape,lx,ly,thick,clarity,frost,glow,fillImg){
  ...
  /* quadratic falloff: clear glass is TRULY transparent (near-zero haze),
     only tinted glass carries colour. bodyA ~0.02 at cl=0.9, ~0.97 at cl=0 */
  const bodyA=jelly?0.95:(0.02+(1-cl)*(1-cl)*0.95);
  glassLightShadow(g,S,path,cB,thick);
  if(glow>0.01)glassGlow(g,S,path,jelly?cA:cB,k,glow);
  g.save();
  ...
  glassGloss(g,S,path,m,k,lx,ly,frost,cl);
}
...
function renderVividGlass(g,S,rng,path,shape,cA,cB,cC,lx,ly,thick,clarity,frost,glow,fillImg){
  const k=S/512,m=shapeMetrics(shape,S);
  const cl=clarity===undefined?0.5:clarity;
  const va=0.06+(1-cl)*(1-cl)*0.9;           /* vivid: near-clear … opaque (quadratic) */
  glassLightShadow(g,S,path,cB,thick);
  if(glow>0.01)glassGlow(g,S,path,cB,k,glow);
  /* deep soft shadow */
  g.save();
...
function renderIcon(ctx,S,spec){
  ...
  if(glassy && spec.bg!=="transparent" && clarity>0.12){
    const zoom=1.0+0.05*clarity;                       /* gentle lens (keeps interior = backdrop) */
    const blurPx=S*(0.002+0.06*frost);                 /* clear=crisp, frosted=heavy */
    const off0=S*(zoom-1)/2;
    const bl=document.createElement("canvas");bl.width=S;bl.height=S;
    const blg=bl.getContext("2d");blg.filter=`blur(${blurPx}px)`;
    blg.drawImage(ctx.canvas,-off0,-off0,S*zoom,S*zoom);blg.filter="none";
    /* paint the refracted backdrop inside the glass — REPLACES the interior
       (never brightens it), so clear glass reads exactly as see-through */
...
function applySpec(sp,target){
  ...
  for(const k of ["style","shape","glyphStyle","bg","baseColor","colorB","colorC",
    ...
    "light","glassThick","puff","clarity","frost","transp","glow"])
