...
function glassGloss(g,S,path,m,k,lx,ly,frost){
  ...
  /* thin top-edge reflection (NOT a haze band) — hugs the top, fades fast */
  const gl=g.createLinearGradient(0,m.y-2*k,0,m.y+m.h*0.2);
  gl.addColorStop(0,`rgba(255,255,255,${0.4*(1-0.5*fr)})`);
  gl.addColorStop(0.5,`rgba(255,255,255,${0.05*(1-0.5*fr)})`);
